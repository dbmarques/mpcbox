function [C,MPC,phi,S12,S1,S2,t,f,confC,phistd,Cerr]=mtmpcgramc(data1,data2,movingwin,params)
% Multi-taper time-frequency coherence,cross-spectrum and individual spectra - continuous processes
%
% Usage:
%
% [C,MPC,phi,S12,S1,S2,t,f,confC,phistd,Cerr]=mtmpcgramc(data1,data2,movingwin,params)
% Input: 
...
for n=1:nw;
   ...
   if nargout==10;
     [c,mpc,ph,s12,s1,s2,f,confc,phie,cerr]=mtmpcspectrum(datawin1,datawin2,params);
...
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
   ...
   elseif nargout==9;
     [c,mpc,ph,s12,s1,s2,f,confc,phie]=mtmpcspectrum(datawin1,datawin2,params);
%      phierr(1,n,:,:)=squeeze(phie(1,:,:));
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
      phistd(n,:,:)=phie;
   else
     [c,mpc,ph,s12,s1,s2,f]=mtmpcspectrum(datawin1,datawin2,params);
   end;
   C(n,:,:)=c;
   MPC(n,:,:)=mpc;
   S12(n,:,:)=s12;
...
C=squeeze(C); MPC=squeeze(MPC); phi=squeeze(phi);S12=squeeze(S12); S1=squeeze(S1); S2=squeeze(S2);
