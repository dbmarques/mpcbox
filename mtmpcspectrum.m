function [C,mpc,phi,S12,S1,S2,f,confC,phistd,Cerr]=mtmpcspectrum(data1,data2,params)
% Multi-taper coherency,cross-spectrum and individual spectra - continuous process
%
% Usage:
% [C,mpc,phi,S12,S1,S2,f,confC,phistd,Cerr]=mtmpcspectrum(data1,data2,params)
% Input: 
...
J12=J1.*conj(J2);
mpc = squeeze(abs(mean(exp(1i*angle(J12)),2)));

if trialave; S12=squeeze(mean(S12,2)); S1=squeeze(mean(S1,2)); S2=squeeze(mean(S2,2)); mpc=squeeze(mean(mpc,2)); end;
C12=S12./sqrt(S1.*S2);
C=abs(C12); 
phi=angle(C12);
if nargout>=8; 
